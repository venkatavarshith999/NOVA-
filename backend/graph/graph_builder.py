...
def build_graph_for_user(db: Session, user_id: str, document_ids: Optional[List[str]] = None) -> nx.MultiDiGraph:
    g = nx.MultiDiGraph()

    doc_query = db.query(Document.id).filter(Document.owner_id == user_id)
    if document_ids:
    ...
    for e in entities:
        ...
        if key not in name_to_id:
            name_to_id[key] = e.id
            g.add_node(e.id, label=e.name, type=e.type, document_id=e.document_id)

    relationships = db.query(Relationship).filter(Relationship.document_id.in_(doc_ids)).all()
    for r in relationships:
        ...
        if not src_id:
            ...
            if src_id not in g:
                g.add_node(src_id, label=r.source_entity, type="Entity", document_id=r.document_id)
        if not tgt_id:
            tgt_id = f"implicit::{tgt_key}"
            if tgt_id not in g:
                g.add_node(tgt_id, label=r.target_entity, type="Entity", document_id=r.document_id)
        g.add_edge(src_id, tgt_id, key=r.id, label=r.relation_type, confidence=r.confidence, id=r.id)
...
def graph_to_api(g: nx.MultiDiGraph) -> dict:
    ...
    stats = {
        ...
        "connected_components": nx.number_weakly_connected_components(g) if g.number_of_nodes() else 0,
    }
    return {"nodes": nodes, "edges": edges, "stats": stats}


def search_graph_for_terms(g: nx.MultiDiGraph, terms: List[str], hops: int = 1) -> List[str]:
...
def detect_compliance_risks(g: nx.MultiDiGraph) -> List[dict]:
    ...
    for pid in policies:
        ...
        if not has_governing:
            ...
            risk_id += 1
            risks.append({
                "id": f"risk-{risk_id}", "severity": "high",
                "title": f"Policy '{label}' has no linked regulation",
                "description": f"The policy '{label}' is not connected to any regulation or compliance standard in the knowledge graph. This may indicate a governance gap.",
                "affected_entities": [label],
                "recommendation": "Link this policy to the applicable regulation or standard to ensure compliance traceability."
            })

    # --- 2. Storage locations without encryption ---
    storage_nodes = [n for n, d in g.nodes(data=True) if d.get("type") in ("Storage Location", "Entity") and "storage" in str(d.get("label", "")).lower()]
    for sid in storage_nodes:
        has_encryption = False
        for neighbor in list(g.successors(sid)) + list(g.predecessors(sid)):
            ntype = g.nodes[neighbor].get("type", "")
            for _, _, edata in list(g.edges(sid, data=True)) + list(g.edges(neighbor, data=True)):
        ...
        if not has_encryption:
            ...
            risks.append({
                ...
                "description": f"The storage location '{label}' is not linked to any encryption standard. Data at rest may be unprotected.",
                "affected_entities": [label],
                "recommendation": "Verify that data stored in this location is encrypted and link the appropriate encryption standard."
            })

    # --- 3. Isolated entities (degree 0 or 1) ---
    for node_id, data in g.nodes(data=True):
        ...
        if deg == 0 and ntype in ("Regulation", "Policy", "Security Control"):
            risk_id += 1
            risks.append({
                "id": f"risk-{risk_id}", "severity": "medium",
                "title": f"Isolated {ntype}: '{label}'",
                "description": f"The {ntype.lower()} '{label}' has no connections in the knowledge graph. It may be referenced in documents but not linked to relevant entities.",
                "affected_entities": [label],
                "recommendation": f"Review the source document to identify and create relationships for this {ntype.lower()}."
            })
    ...
    for cid in controls:
        ...
        if not has_policy:
            ...
            risks.append({
                ...
                "description": f"The security control '{label}' exists in the graph but is not connected to any policy or regulation.",
                "affected_entities": [label],
                "recommendation": "Connect this control to the relevant policy to ensure proper governance coverage."
            })
    ...
    for u, v, data in g.edges(data=True):
        if data.get("label") == "violates":
            ...
            risks.append({
                ...
                "description": f"A violation relationship was identified between '{src_label}' and '{tgt_label}'. This requires immediate attention.",
...
def impact_analysis(g: nx.MultiDiGraph, entity_name: str) -> dict:
    ...
    source_id = None
    for node_id, data in g.nodes(data=True):
        if str(data.get("label", "")).strip().lower() == entity_name.strip().lower():
            source_id = node_id
            break
    if source_id is None:
        # Try partial match
        for node_id, data in g.nodes(data=True):
            if entity_name.lower() in str(data.get("label", "")).lower():
                source_id = node_id
                break
    ...
    # BFS — hop 1 = direct, hop 2 = indirect
    directly_affected = []
    indirectly_affected = []
    ...
    # Indirect neighbors (hop 2)
    indirect_ids = set()
    for did in direct_ids:
        for neighbor in list(g.successors(did)) + list(g.predecessors(did)):
            if neighbor == source_id or neighbor in direct_ids or neighbor in indirect_ids:
                continue
    ...
    return {"directly_affected": directly_affected, "indirectly_affected": indirectly_affected}
